...
const CarDetails = ({ isOpen, closeModal, car }: CarDetailsProps) => {
  ...
  const handlePropertySearch=(key: string,value: any)=>{
                ...
                <div className='flex-1 flex flex-col gap-2'>
                  ...
                  <div className='mt-3 flex flex-wrap gap-4'>
                    {Object.entries(car).map(([key, value]) => (
                      <div className='flex justify-between gap-5 w-full text-right hover:scale-110 transition ease-in-out' key={key} >
                        <h4 className='text-grey capitalize'>
                          {key.split("_").join(" ")}
                        </h4>
                        <p className='text-black-100 font-semibold' onClick={()=>handlePropertySearch(key,value)}>
